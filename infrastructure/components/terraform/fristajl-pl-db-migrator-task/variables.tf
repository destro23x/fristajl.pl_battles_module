...
variable "db_user" {
  type        = string
  description = "IAM-authenticated database role name used to connect (e.g. migration_user_fristajl)"
}
...
variable "subnet_ids" {
  type        = list(string)
  description = "Subnet IDs the migration task runs in (published to SSM for the workflow to read)"
}

variable "security_group_id" {
  type        = string
  description = "Security group ID attached to the migration task (published to SSM for the workflow to read)"
}
