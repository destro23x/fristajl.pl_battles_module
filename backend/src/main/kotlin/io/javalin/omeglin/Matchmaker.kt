...
object Matchmaker {
    ...
    // Users currently on the Arena with no opponent paired yet (i.e. actively searching).
    fun waitingCount(): Int = queue.count { it.b == null }

    // All users currently connected to the Arena (searching, paired, or exchanging).
    fun onlineCount(): Int = onlineUsers.get()

    fun websocket(ws: WsConfig) {
        ws.onConnect { user -> user.enableAutomaticPings(); onlineUsers.incrementAndGet() }
        ws.onClose { user -> onlineUsers.decrementAndGet(); pairingAbort(user) }
        ws.onMessage { user ->
